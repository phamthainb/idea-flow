...
const IdeaSchema = z.object({
  name: z
    .string()
    .min(1, "Tên ý tưởng không được để trống")
    .transform(stripHtml),
...
export async function suggestNameAction(
  description: string
): Promise<{ suggestedName?: string; justification?: string; error?: string }> {
  if (!description || stripHtml(description).trim().length < 10) {
    return {
      error:
        "Vui lòng nhập mô tả chi tiết hơn (ít nhất 10 ký tự) để AI có thể gợi ý tên.",
    };
  }

  try {
    const result = await aiSuggestName({
      ideaDescription: stripHtml(description),
    });
    return {
...
export async function suggestTagsAction(
  description: string
): Promise<{ suggestedTags?: string[]; error?: string }> {
  if (!description || stripHtml(description).trim().length < 10) {
    return {
      error:
        "Vui lòng nhập mô tả chi tiết hơn (ít nhất 10 ký tự) để AI có thể gợi ý tags.",
    };
  }

  try {
    const result = await aiSuggestTags({
      ideaDescription: stripHtml(description),
    });
...
export async function rewriteDescriptionAction(
  description: string
): Promise<{ rewrittenDescription?: string; error?: string }> {
  if (!description || stripHtml(description).trim().length < 10) {
    return {
      error:
        "Vui lòng nhập mô tả chi tiết hơn (ít nhất 10 ký tự) để AI có thể viết lại.",
    };
  }
  ...
  try {
    ...
    return {
      error: "Không thể viết lại mô tả vào lúc này. Vui lòng thử lại sau.",
    };
  }
}
